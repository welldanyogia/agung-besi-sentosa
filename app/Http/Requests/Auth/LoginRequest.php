...
//        }
        if (!Auth::attempt([$loginField => $this->input('login'), 'password' => $this->input('password')], $this->boolean('remember'))) {
            $attempts = RateLimiter::hit($this->throttleKey()); // Tambah jumlah percobaan

            throw ValidationException::withMessages([
                'login' => trans('auth.failed') . " Percobaan ke-{$attempts} dari 5.",
                'too_many_attempts' => $attempts >= 5, // Kirimkan status ke frontend
            ]);
        }
